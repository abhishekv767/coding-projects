...
//    assign D[0] = (Q[0] & ~go) | (Q[5] & go) | (Q[4] & T1);
    ...
    //ship fall
    assign D[1] = (go & Q[0]) | (Q[1] & ~collisionShipWithFloor) | (Q[3] &  T3) | (Q[4] & T1);
    
    //ship sit on floor for one second and disappear
    assign D[2] = (Q[1] & collisionShipWithFloor) | (Q[2] & ~T1);
    
    //slug sit on floor for three secs and disappears
    assign D[3] = (Q[2] & T1) | (Q[3] & ~(T3|collisionPinkSlugWithPlayer));
    
    //picks up slug
    assign D[4] = (Q[3] & collisionPinkSlugWithPlayer) | (Q[4] & ~T1);
    
    //Right Loses round
    assign D[5] = (Q[1] & collisionShipWithPlayer) | (Q[2] & collisionShipWithPlayer) | (Q[5] & ~go);
    
    assign loadTarget = (go & Q[0])  | (Q[3] &  T3) | (Q[4] & T1);
    assign resetTimer = (go & Q[0]) | (Q[1] & collisionShipWithFloor) | (Q[2] & T1) | (Q[3] & collisionPinkSlugWithPlayer)| (Q[4] & T1);// | (Q[3] & T3);// | (Q[3] & collisionPinkSlugWithPlayer);
    assign scoreUp = Q[4] & T1;
    assign shipReset = (Q[3] &  T3) | (Q[4] & T1);
...
module State_Machine(
    ...
    SMachine_Logic Logic(.clk(clk), .btnU(btnU), .go(go), .Q(Q), .collisionPinkSlugWithPlayer(collisionPinkSlugWithPlayer),
