 ...
 * @param none
 * @return TRUE or FALSE
 * @brief This function is a prototype event checker that checks the light level
 * @note
 * @author Gabriel H Elkaim, 2013.09.27 09:18
 * @modified Gabriel H Elkaim/Max Dunne, 2016.09.12 20:08 */
#define HI_THRESHOLD 530;
#define LO_THRESHOLD 470;

//static enum {
 //   DARK, LIGHT
//} lastLightState;
//lastLightState = DARK;
static unsigned int lastLightState = LIGHT_OFF;
static unsigned int Threshold = 530;

uint8_t CheckLightLevel(void) {

	ES_Event ThisEvent;
    unsigned int currentLightState;
    ...
    if (currentLightState != lastLightState) { //event detected
        ...
        ThisEvent.EventParam = (uint16_t) Roach_LightLevel();
        returnVal = TRUE;
        PostTemplateHSM(ThisEvent);
    }
...
/**
 * @Function checkBumpEvent(void)
 * @param none
 * @return TRUE or FALSE
 * @brief This function is a prototype event checker that checks the bump event
 * @note 
 * @author Gabriel H Elkaim, 2013.09.27 09:18
 * @modified Gabriel H Elkaim/Max Dunne, 2016.09.12 20:08 */
uint8_t CheckBumpEvent(void) {
    static ES_EventTyp_t lastEvent = BUMPED_NOT;
    ES_EventTyp_t curEvent;
    ES_Event thisEvent;
    uint8_t returnVal = FALSE;
    //uint16_t bumpState = Roach_ReadBumpers(void); // fix this

    if (Roach_ReadBumpers() == 0x03 || Roach_ReadBumpers() == 0x02 || Roach_ReadBumpers() == 0x01) { // is battery connected?
        curEvent = FBUMPED;
    }
    else if (Roach_ReadBumpers() == 0x0C || Roach_ReadBumpers() == 0x08 || Roach_ReadBumpers() == 0x04){
        curEvent = BBUMPED;
    }else {
        curEvent = BUMPED_NOT;
    }
    if (curEvent != lastEvent) { // check for change from last time
        thisEvent.EventType = curEvent;
        thisEvent.EventParam = Roach_ReadBumpers();
        returnVal = TRUE;
        lastEvent = curEvent; // update history
#ifndef EVENTCHECKER_TEST           // keep this as is for test harness
        PostTemplateHSM(thisEvent);
