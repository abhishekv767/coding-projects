...
ES_Event RunTemplateFSM(ES_Event ThisEvent) {
    ...
    switch (CurrentState) {
        case InitPState: // If current state is initial Psedudo State
            ...
            {
                nextState = FirstState; // dark
                makeTransition = TRUE;
                ThisEvent.EventType = ES_NO_EVENT;
            }
            break;

        case FirstState: //move forward
            Roach_LeftMtrSpeed(80);
            Roach_RightMtrSpeed(75);
            if (ThisEvent.EventType == FBUMPED || ThisEvent.EventType == BBUMPED)// only respond to ES_Init
            {
                ES_Timer_InitTimer(SIMPLE_SERVICE_TIMER, TIMER_1_TICKS);
        ...
        case SecondState: //going back
            Roach_LeftMtrSpeed(80);
            Roach_RightMtrSpeed(-80);
            if (ThisEvent.EventType == ES_TIMEOUT)// only respond to ES_Init
            {
                ES_Timer_StopTimer(SIMPLE_SERVICE_TIMER);
                nextState = FirstState;
                makeTransition = TRUE;
                ThisEvent.EventType = ES_NO_EVENT;
            }
            if (ThisEvent.EventType == LIGHT_OFF)// only respond to ES_Init
            {
                nextState = ThirdState;
                makeTransition = TRUE;
                ThisEvent.EventType = ES_NO_EVENT;
        ...
        case ThirdState: //hide
            Roach_LeftMtrSpeed(0);
            Roach_RightMtrSpeed(0);
            if (ThisEvent.EventType == LIGHT_ON)// only respond to ES_Init
            {
                nextState = FirstState;
                makeTransition = TRUE;
                ThisEvent.EventType = ES_NO_EVENT;
