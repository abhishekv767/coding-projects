...
ES_Event RunTemplateService(ES_Event ThisEvent)
{
    ...
    uint16_t bumpTester = Roach_ReadBumpers(); // read the battery voltage
    ...
    case ES_TIMEOUT:
        ...
        if (bumpTester != 0x00) { // is battery connected?
            curEvent = FBUMPED;
        } else {
            curEvent = BUMPED_NOT;
        }
        if (curEvent != lastEvent) { // check for change from last time
            ...
            ReturnEvent.EventParam = bumpTester;
